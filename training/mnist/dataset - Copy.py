...
class MNIST(DataSet):
    ...
    def __init__(self, original="true", reconstruct='true', comps=10, val_size=1000, seed=9) -> None:
        ...
        if(original!="true"):
            ...
            # this one need to be scaled when ss applied
            U, s, V = np.linalg.svd(x_train[:10000])
            x_train_proj=np.dot(x_train,V[:num_components,:].T)
            
            x_test=x_test.reshape((10000,-1))
            x_test_proj=np.dot(x_test,V[:num_components,:].T) 
                
            #self.m=np.mean(x_train_proj,axis=0)
            #self.sigma=np.std(x_train_proj,axis=0)
            self.m=scaler.mean_
            self.sigma=scaler.std_
            
            V = V
           
            if(reconstruct=="true"):
                print('shape',self.m.shape,self.sigma.shape)
                x_train=np.dot(x_train_proj,V[:num_components,:]) #+ scaler.mean_
                x_test=   np.dot(x_test_proj,V[:num_components,:])
        ...
        self.x_train_original = x_train_original.reshape((x_train_original.shape[0], 28, 28, 1))
        self.x_test_original = x_test_original.reshape((x_test_original.shape[0], 28, 28, 1))
        self.x_train_original, self.y_train, self.x_val, self.y_val = self.split_data(self.rnd, val_size // 10, self.x_train_original, self.y_train)
    ...
    def get_train(self):
        #return the original training data without scaling, because the model will apply it
        return self.x_train_original, self.y_train

    def get_test(self):
        #return the original training data without scaling, because the model will apply it
        return self.x_test_original, self.y_test
